    ...
    for pos in grid:
        if grid[pos] == '*':
            ...
            for number, positions in numbers:
                if any(taxi_distance(pos, num_pos) == 1 or max(abs(pos[0] - num_pos[0]), abs(pos[1] - num_pos[1])) == 1 for num_pos in positions):
...
answer(11.2, 791134099634, lambda: calculate_galaxy_distances(in11, expansion_factor=1000000))
...
def count_arrangements(pattern, groups):
    ...
    # pattern[0] == '?'
    return (count_arrangements('#' + pattern[1:], groups) + 
            count_arrangements('.' + pattern[1:], groups))
...
def solve_lens_library(sequence, part2=False):
    ...
    boxes = [[] for _ in range(256)]  # Each box is a list of (label, focal_length) pairs
...
def trace_light_beam(grid, start_pos, start_dir):
    ...
    while queue:
        ...
        if tile == '.':
            ...
            # Mirror /
            direction_map = {'right': 'up', 'left': 'down', 'up': 'right', 'down': 'left'}
            next_moves.append(direction_map[direction])
        elif tile == '\\':
            # Mirror \
            direction_map = {'right': 'down', 'left': 'up', 'up': 'left', 'down': 'right'}
            next_moves.append(direction_map[direction])
...
def solve_lava_floor(lines, find_maximum=False):
    ...
    for x in range(width):
        ...
        max_energized = max(max_energized, trace_light_beam(grid, (x, height - 1), 'up'))
    
    # Left and right edges
    for y in range(height):
        max_energized = max(max_energized, trace_light_beam(grid, (0, y), 'right'))
        max_energized = max(max_energized, trace_light_beam(grid, (width - 1, y), 'left'))
